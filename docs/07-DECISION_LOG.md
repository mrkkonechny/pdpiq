...
- **Consequences:** Review count pass threshold changes from ≥25 to ≥50 — existing analyses will show different Authority & Trust scores on re-run. lastModified and factualSpecificity are new factors; history entries from v2.x will not have these scores. Platform divergence note is informational only — no scoring impact.
